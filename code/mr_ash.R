...
mr_ash_update_cd <- function (X, y, b, se, s0, w0) {
  ...
  for (i in 1:p) {
    ...
    erssi <- norm2(r)^2 + v
    d     <- elbo_const - (out$logbf + loglik0 + erssi/(2*se))
    elbo  <- elbo - d
    erss  <- erss + v
  }
...
mr_ash_update_optim <- function (X, y, b, se, s0, w0,
  ...
  # Recover the parameters we are optimizing.
  xx   <- diag(crossprod(X))
  r    <- drop(y - X %*% b)
  vars <- drop(r %*% X)/xx
  ...
  # bound.
  if (method == "nm")
    out <- optim(vars,fn,method = method,control = list(reltol = 1e-12))
  else if (method == "bfgs") {
    # TO DO.
  }
...
compute_elbo_ss <- function (X, y, b, se) {
...
bayes_lr_mix_ss <- function (bhat, shat, se, s0, w0) {
  ...
  # (logbf).
  return(list(mu1 = mu1,s1 = drop(s1),w1 = w1,logbf = logbf))
}
