...
```{r}
plot1 <- ggplot(res,aes(x = rmse.min,y = rmse.1se,fill = p1)) +
...
```{r}
plot2 <- ggplot(res,aes(x = rmse.min,y = rmse.mrash2,fill = p1)) +
  geom_point(size = 2,shape = 21,color = "white") +
  geom_abline(slope = 1,intercept = 0,color = "black",linetype = "dotted") +
  scale_fill_gradient2(low = "lightskyblue",mid = "gold",high = "orangered",
                       midpoint = 250) +
...
```{r}
plot3 <- ggplot(res,aes(x = rmse.mrash1,y = rmse.mrash2,fill = p1)) +
  geom_point(size = 2,shape = 21,color = "white") +
  geom_abline(slope = 1,intercept = 0,color = "black",linetype = "dotted") +
  scale_fill_gradient2(low = "lightskyblue",mid = "gold",high = "orangered",
                       midpoint = 250) +
...
fit.glmnet <- cv.glmnet(X,y,alpha = 0.95,standardize = FALSE)
fit.mrash1 <- mr.ash(X,y,standardize = FALSE)
fit.mrash2 <- mr.ash(X,y,update.pi = FALSE,standardize = FALSE,
                     sa2 = c(0,1/80),pi = c(0.5,0.5))
